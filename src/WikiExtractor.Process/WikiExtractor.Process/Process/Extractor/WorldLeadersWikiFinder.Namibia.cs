...
namespace WikiExtractor.Process.Extractor
{
    ...
    {
        public List<WikiWhatToExtractModel> ExtractListTabularData_Namibia(HtmlDocument document, List<string>? tags)
        {
            ...
            foreach (var table in tableData)
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var r = ExtractListTabularData_Namibia_Rows(cells);
        ...
        private WikiWhatToExtractModel? ExtractListTabularData_Namibia_Rows(HtmlNode[] elements)
        {
            var m = new WikiWhatToExtractModel();
            m.AdditionalMetaData!.Add("Country", "Namibia");
            int c = 1;
            foreach (var elm in elements)
            {
                // td1=portrait, td2=name, td3=election, td4=took, td5=left, td6=tenure, td7=party
