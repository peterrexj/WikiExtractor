...
namespace WikiExtractor.Process.Extractor
{
    ...
    {
        public List<WikiWhatToExtractModel> ExtractListTabularData_Estonia(HtmlDocument document, List<string>? tags)
        {
            ...
            foreach (var table in tableData)
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var r = ExtractListTabularData_Estonia_Rows(cells);
        ...
        private WikiWhatToExtractModel? ExtractListTabularData_Estonia_Rows(HtmlNode[] elements)
        {
            var m = new WikiWhatToExtractModel();
            m.AdditionalMetaData!.Add("Country", "Estonia");
            int c = 1;
            foreach (var elm in elements)
            {
                // ordinal=TH(skipped), td1=portrait, td2=name (no birth-death in name cell), td3=took, td4=left
