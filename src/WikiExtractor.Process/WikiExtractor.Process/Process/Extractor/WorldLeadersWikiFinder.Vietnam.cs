...
namespace WikiExtractor.Process.Extractor
{
    ...
    {
        public List<WikiWhatToExtractModel> ExtractListTabularData_Vietnam(HtmlDocument document, List<string>? tags)
        {
            ...
            foreach (var table in tableData.Take(2))
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var r = ExtractListTabularData_Vietnam_Rows(cells);
        ...
        private WikiWhatToExtractModel? ExtractListTabularData_Vietnam_Rows(HtmlNode[] elements)
        {
            var m = new WikiWhatToExtractModel();
            m.AdditionalMetaData!.Add("Country", "Vietnam");
            int c = 1;
            foreach (var elm in elements)
            {
                // td1=portrait, td2=name, td3=took, td4=left, td5=tenure, td6=party
