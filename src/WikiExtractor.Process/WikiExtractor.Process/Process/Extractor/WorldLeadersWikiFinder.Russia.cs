...
namespace WikiExtractor.Process.Extractor
{
    ...
    {
        public List<WikiWhatToExtractModel> ExtractListTabularData_Russia(HtmlDocument document, List<string>? tags)
        {
            ...
            foreach (var table in tableData)
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var r = ExtractListTabularData_Russia_Rows(cells);
        ...
        private WikiWhatToExtractModel? ExtractListTabularData_Russia_Rows(HtmlNode[] elements)
        {
            var m = new WikiWhatToExtractModel();
            m.AdditionalMetaData!.Add("Country", "Russia");
            int c = 1;
            foreach (var elm in elements)
            {
                // 2 THs skipped (blank+ordinal), td1=portrait, td2=name (bare YYYY–YYYY no parens), td3=combined term+tenure
