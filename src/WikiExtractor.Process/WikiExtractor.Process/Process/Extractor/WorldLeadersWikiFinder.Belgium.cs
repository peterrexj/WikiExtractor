...
namespace WikiExtractor.Process.Extractor
{
    ...
    {
        public List<WikiWhatToExtractModel> ExtractListTabularData_Belgium(HtmlDocument document, List<string>? tags)
        {
            ...
            foreach (var table in tableData)
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var r = ExtractListTabularData_Belgium_Rows(cells);
        ...
        private WikiWhatToExtractModel? ExtractListTabularData_Belgium_Rows(HtmlNode[] elements)
        {
            var m = new WikiWhatToExtractModel();
            m.AdditionalMetaData!.Add("Country", "Belgium");
            int c = 1;
            foreach (var elm in elements)
            {
                // ordinal=TH(skipped), td1=portrait, td2=name+lifespan, td3=election, td4=took, td5=left, td6=tenure, td7=party
