...
namespace WikiExtractor.Process.Extractor
{
    ...
    {
        public List<WikiWhatToExtractModel> ExtractListTabularData_ElSalvador(HtmlDocument document, List<string>? tags)
        {
            ...
            foreach (var table in tableData)
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var r = ExtractListTabularData_ElSalvador_Rows(cells);
        ...
        private WikiWhatToExtractModel? ExtractListTabularData_ElSalvador_Rows(HtmlNode[] elements)
        {
            var m = new WikiWhatToExtractModel();
            m.AdditionalMetaData!.Add("Country", "El Salvador");
            int c = 1;
            foreach (var elm in elements)
            {
                // td1=portrait, td2=name, td3=ordinal, td4=took, td5=left, td6=tenure, td7=color, td8=party
