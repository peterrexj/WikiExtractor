...
namespace WikiExtractor.Process.Extractor
{
    ...
    {
        public List<WikiWhatToExtractModel> ExtractListTabularData_Uruguay(HtmlDocument document, List<string>? tags)
        {
            ...
            foreach (var table in tableData)
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var r = ExtractListTabularData_Uruguay_Rows(cells);
        ...
        private WikiWhatToExtractModel? ExtractListTabularData_Uruguay_Rows(HtmlNode[] elements)
        {
            var m = new WikiWhatToExtractModel();
            m.AdditionalMetaData!.Add("Country", "Uruguay");
            int c = 1;
            foreach (var elm in elements)
            {
                // td1=portrait, td2=name+lifespan, td3=took, td4=left
