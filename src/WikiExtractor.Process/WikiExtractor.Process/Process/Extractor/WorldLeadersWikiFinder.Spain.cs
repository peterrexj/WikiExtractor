...
namespace WikiExtractor.Process.Extractor
{
    ...
    {
        public List<WikiWhatToExtractModel> ExtractListTabularData_Spain(HtmlDocument document, List<string>? tags)
        {
            ...
            foreach (var table in tableData)
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var r = ExtractListTabularData_Spain_Rows(cells);
        ...
        private WikiWhatToExtractModel? ExtractListTabularData_Spain_Rows(HtmlNode[] elements)
        {
            var m = new WikiWhatToExtractModel();
            m.AdditionalMetaData!.Add("Country", "Spain");
            int c = 1;
            foreach (var elm in elements)
            {
                // all td: td1=portrait, td2=name+lifespan, td3=took, td4=left, td5=tenure, td6=empty, td7=party
