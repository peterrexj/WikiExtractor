...
namespace WikiExtractor.Process.Extractor
{
    ...
    {
        public List<WikiWhatToExtractModel> ExtractListTabularData_Ireland(HtmlDocument document, List<string>? tags)
        {
            ...
            foreach (var table in tableData)
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var r = ExtractListTabularData_Ireland_Rows(cells);
        ...
        private WikiWhatToExtractModel? ExtractListTabularData_Ireland_Rows(HtmlNode[] elements)
        {
            var m = new WikiWhatToExtractModel();
            m.AdditionalMetaData!.Add("Country", "Ireland");
            int c = 1;
            foreach (var elm in elements)
            {
                // color-style TH skipped, td1=portrait, td2=name+lifespan, td3=took, td4=left, td5=tenure, td6=party
