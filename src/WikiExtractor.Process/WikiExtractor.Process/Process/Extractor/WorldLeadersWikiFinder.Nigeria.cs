...
namespace WikiExtractor.Process.Extractor
{
    ...
    {
        public List<WikiWhatToExtractModel> ExtractListTabularData_Nigeria(HtmlDocument document, List<string>? tags)
        {
            ...
            foreach (var table in tableData)
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var r = ExtractListTabularData_Nigeria_Rows(cells);
        ...
        private WikiWhatToExtractModel? ExtractListTabularData_Nigeria_Rows(HtmlNode[] elements)
        {
            var m = new WikiWhatToExtractModel();
            m.AdditionalMetaData!.Add("Country", "Nigeria");
            int c = 1;
            foreach (var elm in elements)
            {
                // ordinal=TH(skipped), td1=portrait, td2=name, td3=took, td4=left, td5=tenure, td6=party
