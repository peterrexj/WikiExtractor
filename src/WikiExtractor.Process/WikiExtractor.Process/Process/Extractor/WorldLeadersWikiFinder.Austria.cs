...
namespace WikiExtractor.Process.Extractor
{
    ...
    {
        public List<WikiWhatToExtractModel> ExtractListTabularData_Austria(HtmlDocument document, List<string>? tags)
        {
            ...
            foreach (var table in tableData)
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var r = ExtractListTabularData_Austria_Rows(cells);
        ...
        private WikiWhatToExtractModel? ExtractListTabularData_Austria_Rows(HtmlNode[] elements)
        {
            var m = new WikiWhatToExtractModel();
            m.AdditionalMetaData!.Add("Country", "Austria");
            int c = 1;
            foreach (var elm in elements)
            {
                // ordinal=TH+mid-TH(scope=row) skipped; td1=portrait, td2=name+lifespan, td3=took, td4=left, td5=tenure, td6=party, td7=election
