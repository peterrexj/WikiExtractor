...
namespace WikiExtractor.Process.Extractor
{
    ...
    {
        public List<WikiWhatToExtractModel> ExtractListTabularData_Finland(HtmlDocument document, List<string>? tags)
        {
            ...
            foreach (var table in tableData)
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var r = ExtractListTabularData_Finland_Rows(cells);
        ...
        private WikiWhatToExtractModel? ExtractListTabularData_Finland_Rows(HtmlNode[] elements)
        {
            var m = new WikiWhatToExtractModel();
            m.AdditionalMetaData!.Add("Country", "Finland");
            int c = 1;
            foreach (var elm in elements)
            {
                // all td: td1=ordinal, td2=portrait, td3=name+lifespan, td4=took, td5=left, td6=tenure, td7=election, td8=empty, td9=party
                ...
                if (c == 9) Common_SimpleDataType01_Extract(elm, m, "Political Party", removeSpecialChars: false);
                c++;
            }
