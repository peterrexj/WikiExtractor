...
namespace WikiExtractor.Process.Extractor
{
    ...
    {
        public List<WikiWhatToExtractModel> ExtractListTabularData_Mozambique(HtmlDocument document, List<string>? tags)
        {
            ...
            foreach (var table in tableData)
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var r = ExtractListTabularData_Mozambique_Rows(cells);
        ...
        private WikiWhatToExtractModel? ExtractListTabularData_Mozambique_Rows(HtmlNode[] elements)
        {
            var m = new WikiWhatToExtractModel();
            m.AdditionalMetaData!.Add("Country", "Mozambique");
            int c = 1;
            foreach (var elm in elements)
            {
                // td1=ordinal, td2=portrait, td3=name, td4=took, td5=left, td6=tenure, td7=party
