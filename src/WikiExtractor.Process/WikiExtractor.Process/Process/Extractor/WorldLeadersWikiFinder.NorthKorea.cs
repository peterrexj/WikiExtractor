...
namespace WikiExtractor.Process.Extractor
{
    ...
    {
        public List<WikiWhatToExtractModel> ExtractListTabularData_NorthKorea(HtmlDocument document, List<string>? tags)
        {
            ...
            foreach (var table in tableData.Skip(2).Take(1))
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var r = ExtractListTabularData_NorthKorea_Rows(cells);
        ...
        private WikiWhatToExtractModel? ExtractListTabularData_NorthKorea_Rows(HtmlNode[] elements)
        {
            var m = new WikiWhatToExtractModel();
            m.AdditionalMetaData!.Add("Country", "North Korea");
            int c = 1;
            foreach (var elm in elements)
            {
                // td1=color swatch (skip), td2=name, td3=took, td4=left, td5=party (no portrait, no birth-death)
