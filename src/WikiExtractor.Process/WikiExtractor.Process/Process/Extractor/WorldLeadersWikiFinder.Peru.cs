...
namespace WikiExtractor.Process.Extractor
{
    ...
    {
        public List<WikiWhatToExtractModel> ExtractListTabularData_Peru(HtmlDocument document, List<string>? tags)
        {
            ...
            foreach (var table in tableData)
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var r = ExtractListTabularData_Peru_Rows(cells);
        ...
        private WikiWhatToExtractModel? ExtractListTabularData_Peru_Rows(HtmlNode[] elements)
        {
            var m = new WikiWhatToExtractModel();
            m.AdditionalMetaData!.Add("Country", "Peru");
            int c = 1;
            foreach (var elm in elements)
            {
                // td1=portrait, td2=name, td3=took, td4=left, td5=tenure, td6=empty(color), td7=party
