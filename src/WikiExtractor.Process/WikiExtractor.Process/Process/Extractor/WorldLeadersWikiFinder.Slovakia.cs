...
namespace WikiExtractor.Process.Extractor
{
    ...
    {
        public List<WikiWhatToExtractModel> ExtractListTabularData_Slovakia(HtmlDocument document, List<string>? tags)
        {
            ...
            foreach (var table in tableData.Take(2))
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var r = ExtractListTabularData_Slovakia_Rows(cells);
        ...
        private WikiWhatToExtractModel? ExtractListTabularData_Slovakia_Rows(HtmlNode[] elements)
        {
            var m = new WikiWhatToExtractModel();
            m.AdditionalMetaData!.Add("Country", "Slovakia");
            int c = 1;
            foreach (var elm in elements)
            {
                // all td: td1=ordinal, td2=portrait, td3=name, td4=took, td5=left, td6=tenure, td7=party
