...
namespace WikiExtractor.Process.Extractor
{
    ...
    {
        public List<WikiWhatToExtractModel> ExtractListTabularData_Philippines(HtmlDocument document, List<string>? tags)
        {
            ...
            foreach (var table in tableData.Take(1))
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var r = ExtractListTabularData_Philippines_Rows(cells);
        ...
        private WikiWhatToExtractModel? ExtractListTabularData_Philippines_Rows(HtmlNode[] elements)
        {
            var m = new WikiWhatToExtractModel();
            m.AdditionalMetaData!.Add("Country", "Philippines");
            int c = 1;
            foreach (var elm in elements)
            {
                // td1=portrait, td2=name, td3=took, td4=left, td5=tenure, td6=empty, td7=party
