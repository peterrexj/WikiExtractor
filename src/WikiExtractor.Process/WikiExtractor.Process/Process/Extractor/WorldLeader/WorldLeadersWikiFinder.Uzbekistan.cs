...
namespace WikiExtractor.Process.Extractor
{
    ...
    {
        public List<WikiWhatToExtractModel> ExtractListTabularData_Uzbekistan(HtmlDocument document, List<string>? tags)
        {
            ...
            foreach (var table in tableData.Skip(7).Take(1))
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var r = ExtractListTabularData_Uzbekistan_Rows(cells);
        ...
        private WikiWhatToExtractModel? ExtractListTabularData_Uzbekistan_Rows(HtmlNode[] elements)
        {
            var m = new WikiWhatToExtractModel();
            m.AdditionalMetaData!.Add("Country", "Uzbekistan");
            int c = 1;
            foreach (var elm in elements)
            {
                // td1=color-swatch, td2=portrait, td3=name, td4=election, td5=took, td6=left, td7=tenure, td8=party
