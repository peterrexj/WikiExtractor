...
namespace WikiExtractor.Process.Extractor
{
    ...
    {
        public List<WikiWhatToExtractModel> ExtractListTabularData_Egypt(HtmlDocument document, List<string>? tags)
        {
            ...
            foreach (var table in tableData)
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var r = ExtractListTabularData_Egypt_Rows(cells);
        ...
        private WikiWhatToExtractModel? ExtractListTabularData_Egypt_Rows(HtmlNode[] elements)
        {
            var m = new WikiWhatToExtractModel();
            m.AdditionalMetaData!.Add("Country", "Egypt");
            int c = 1;
            foreach (var elm in elements)
            {
                // td1=portrait, td2=name, td3=combined "took–left", td4=tenure, td5=party
