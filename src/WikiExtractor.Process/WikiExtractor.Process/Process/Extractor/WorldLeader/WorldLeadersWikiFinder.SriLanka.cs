...
namespace WikiExtractor.Process.Extractor
{
    ...
    {
        public List<WikiWhatToExtractModel> ExtractListTabularData_SriLanka(HtmlDocument document, List<string>? tags)
        {
            ...
            foreach (var table in tableData)
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var r = ExtractListTabularData_SriLanka_Rows(cells);
        ...
        private WikiWhatToExtractModel? ExtractListTabularData_SriLanka_Rows(HtmlNode[] elements)
        {
            var m = new WikiWhatToExtractModel();
            m.AdditionalMetaData!.Add("Country", "Sri Lanka");
            int c = 1;
            foreach (var elm in elements)
            {
                // td1=color, td2=ordinal, td3=portrait, td4=name, td5=took, td6=left, td7=empty, td8=party
                if (c == 3) Common_Portrait_Extract(elm, m);
                if (c == 4) Common_PersonDetail_Extract(elm, m, titleRemoveInnerSpan: false, extractBirthDeath: true);
