...
namespace WikiExtractor.Process.Extractor
{
    ...
    {
        public List<WikiWhatToExtractModel> ExtractListTabularData_Iran(HtmlDocument document, List<string>? tags)
        {
            ...
            foreach (var table in tableData.Take(1))
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var r = ExtractListTabularData_Iran_Rows(cells);
        ...
        private WikiWhatToExtractModel? ExtractListTabularData_Iran_Rows(HtmlNode[] elements)
        {
            var m = new WikiWhatToExtractModel();
            m.AdditionalMetaData!.Add("Country", "Iran");
            int c = 1;
