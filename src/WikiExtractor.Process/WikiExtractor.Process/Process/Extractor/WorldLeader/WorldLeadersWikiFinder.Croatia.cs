...
namespace WikiExtractor.Process.Extractor
{
    ...
    {
        public List<WikiWhatToExtractModel> ExtractListTabularData_Croatia(HtmlDocument document, List<string>? tags)
        {
            ...
            foreach (var table in tableData)
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var r = ExtractListTabularData_Croatia_Rows(cells);
        ...
        private WikiWhatToExtractModel? ExtractListTabularData_Croatia_Rows(HtmlNode[] elements)
        {
            var m = new WikiWhatToExtractModel();
            m.AdditionalMetaData!.Add("Country", "Croatia");
            int c = 1;
            foreach (var elm in elements)
            {
                // td1=CSS-injected text (skip), td2=portrait, td3=name+lifespan, td4=election, td5=took, td6=left, td7=tenure, td8=party
