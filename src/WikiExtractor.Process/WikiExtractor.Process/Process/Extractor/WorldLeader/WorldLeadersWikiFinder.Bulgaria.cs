...
namespace WikiExtractor.Process.Extractor
{
    ...
    {
        public List<WikiWhatToExtractModel> ExtractListTabularData_Bulgaria(HtmlDocument document, List<string>? tags)
        {
            ...
            foreach (var table in tableData)
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var r = ExtractListTabularData_Bulgaria_Rows(cells);
        ...
        private WikiWhatToExtractModel? ExtractListTabularData_Bulgaria_Rows(HtmlNode[] elements)
        {
            var m = new WikiWhatToExtractModel();
            m.AdditionalMetaData!.Add("Country", "Bulgaria");
            int c = 1;
            foreach (var elm in elements)
            {
                // 6-TD rows (table 3): td1=portrait, td2=name, td3=took, td4=left, td5=tenure, td6=notes
                // 7-TD rows (tables 2,4 — modern era with election col): td1=portrait, td2=name, td3=election, td4=took, td5=left, td6=tenure, td7=party
