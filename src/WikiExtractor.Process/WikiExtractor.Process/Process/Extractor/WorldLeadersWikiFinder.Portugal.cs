...
namespace WikiExtractor.Process.Extractor
{
    ...
    {
        public List<WikiWhatToExtractModel> ExtractListTabularData_Portugal(HtmlDocument document, List<string>? tags)
        {
            ...
            foreach (var table in tableData)
            {
                ...
                foreach (var row in rows)
                {
                    ...
                    {
                        var r = ExtractListTabularData_Portugal_Rows(cells);
        ...
        private WikiWhatToExtractModel? ExtractListTabularData_Portugal_Rows(HtmlNode[] elements)
        {
            var m = new WikiWhatToExtractModel();
            m.AdditionalMetaData!.Add("Country", "Portugal");
            int c = 1;
            foreach (var elm in elements)
            {
                // all td: td1=portrait, td2=name+lifespan, td3=took, td4=left, td5=tenure, td6=election, td7=empty, td8=party
